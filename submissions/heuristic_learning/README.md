...
Pair-push notes: a small-benchmark minimal-displacement legalization candidate is gated to `240 <= n_hard <= 320` and `0.30 <= utilization < 0.52`, with a wider gap only for the `ibm09`-like low-skew bucket. It is appended only when hard-legal, then still competes through the official proxy scorer. Integrated validation improved `ibm01` to `1.0054`, `ibm03` to `1.2812`, `ibm04` to `1.2760`, `ibm07` to `1.4322`, `ibm08` to `1.4515`, and `ibm09` to `1.0805`.
